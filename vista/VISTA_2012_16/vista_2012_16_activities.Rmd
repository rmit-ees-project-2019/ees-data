...
## Understanding Stop Data

We use Stop Table (`S_VISTA12_16_SA1_V1.csv`) with these select columns:
```
---------------------------------------------------------------------------
Column            Description
----------------  ---------------------------------------------------------
...
CW_WDSTOPWGT_LGA  Stop weight for an 'Average weekday’ of the combined 
                  2012-14 and 2014-16 ReportingPeriods, using the ASGC.
CW_WESTOPWGT_LGA  Stop weight for an 'Average weekend day' of the combined 
                  2012-14 and 2014-16 ReportingPeriods, using the ASGC.
---------------------------------------------------------------------------
```
## Example Stop Record for a Person

```
--------------------------------------------------------------------------------------------
        PERSID TRAVDOW         ORIGPURP1         DESTPURP1 STARTIME ARRTIME CW_WDSTOPWGT_LGA
-------------- ------- ----------------- ----------------- -------- ------- ----------------
Y12H0000126P01  Friday           At Home      Work Related      475     535           139.78
...
Y12H0000126P01  Friday      Work Related           Go Home     1050    1125           139.78
--------------------------------------------------------------------------------------------
```

## Extracting Activities from Stop Records
...
```
--------------------------------------------------------------------------------------
        PERSID TRAVDOW          ACTIVITY  ACT.START.TIME ACT.END.TIME CW_WDSTOPWGT_LGA
-------------- ---------------- --------- -------------- ------------ ----------------
Y12H0000126P01  Friday           At Home               0          475           139.78
...
Y12H0000126P01  Friday           Go Home            1125         1439           139.78
--------------------------------------------------------------------------------------
```
...
1. For every activity record, count `CW_WDSTOPWGT_LGA` persons in every time bin that overlaps
   with the activity time

1. For any time bins that do not add up to the expected population 
   (sum of `CW_WDSTOPWGT_LGA` per unique person), scale up the time bin vector of activities
